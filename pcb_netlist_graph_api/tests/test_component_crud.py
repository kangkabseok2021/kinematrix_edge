...
def test_add_component_mutation(gql_client: Client) -> None:
    result = gql_client.execute(
        """
        mutation {
            addComponent(mfrPn: "RES-001", lcscCode: "C123456", category: "resistor",
...
@pytest.mark.django_db
def test_list_components_returns_all(gql_client: Client) -> None:
...
@pytest.mark.django_db
def test_list_components_filtered_by_category(gql_client: Client) -> None:
...
@pytest.mark.django_db
def test_add_component_duplicate_mfr_pn_returns_error(gql_client: Client) -> None:
...
@pytest.mark.django_db
def test_component_parameters_round_trip(gql_client: Client) -> None:
