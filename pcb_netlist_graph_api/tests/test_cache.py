...
@pytest.mark.django_db
@override_settings(CACHES=LOCMEM_CACHE)
def test_cache_invalidated_on_component_save(execute: Any) -> None:
    from django.core.cache import cache
    ...
    result1 = execute('{ components(category: "capacitor") { mfrPn } }')
    assert len(result1.data["components"]) == 1
    CapacitorFactory.create(
        mfr_pn="CAP-CACHE-002", parameters={"capacitance_nf": 100, "voltage_v": 16}
    )
    result2 = execute('{ components(category: "capacitor") { mfrPn } }')
